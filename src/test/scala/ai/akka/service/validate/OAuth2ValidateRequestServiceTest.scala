...
import ai.akka.exception.Exception.{InvalidAuthenticationException, UnsupportedResponseTypeException, InvalidClientException}
...
class OAuth2ValidateRequestServiceTest extends Specification {
  ...
  "The OAuth2ValidateRequestService" should {
    "pass authorization request with response_type and client_id" in {
      ...
      OAuth2ValidateRequestServiceForTest.validateAuthorizationRequest(request, Authentication("user", isAuthenticated = true), null) must be equalTo request
    }

    "fail null authorization request" in {
      OAuth2ValidateRequestServiceForTest.validateAuthorizationRequest(null, null, null) must throwA[IllegalArgumentException]
    }
    ...
    "fail authorization request without client_id " in {
      ...
      OAuth2ValidateRequestServiceForTest.validateAuthorizationRequest(request, null, null) must throwA[InvalidClientException]
    }
    ...
    "fail authorization request with null response types" in {
      ...
      OAuth2ValidateRequestServiceForTest.validateAuthorizationRequest(request, null, null) must throwA[UnsupportedResponseTypeException]
    }
    ...
    "fail validation without authentication" in {
      val request: AuthorizationRequest = authorizationRequest("clientId", Set(Constants.TOKEN_RESPONSE_TYPE))
      OAuth2ValidateRequestServiceForTest.validateAuthorizationRequest(request, null, null) must throwA[InvalidAuthenticationException]
    }

    "fail validation if user is not uthenticated" in {
      val request: AuthorizationRequest = authorizationRequest("clientId", Set(Constants.TOKEN_RESPONSE_TYPE))
      OAuth2ValidateRequestServiceForTest.validateAuthorizationRequest(request, Authentication("user", isAuthenticated = false), null) must throwA[InvalidAuthenticationException]
    }
  }
